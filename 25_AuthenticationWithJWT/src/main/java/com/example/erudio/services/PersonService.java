...
public class PersonService {
    ...
    private PagedModel<EntityModel<PersonDTO>> buildPagedModel(Pageable pageable, Page<Person> people) {
        // 1. Mapeia a página de Entidades para uma página de DTOs nativamente pelo Spring Data
        Page<PersonDTO> dtoPage = people.map(person -> {
            var dto = parseObject(person, PersonDTO.class);
            addHateoasLinks(dto);
            return dto;
        });

        // 2. Cria os links básicos de navegação da paginação (self) apontando para o Controller
        Link selfLink = linkTo(methodOn(PersonController.class)
                .findAll(pageable.getPageNumber(), pageable.getPageSize(), "asc"))
                .withSelfRel();

        // 3. O Assembler do Spring HATEOAS 3.x monta o PagedModel ideal para o Jackson serializar sem duplicar links
